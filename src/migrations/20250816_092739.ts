...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   CREATE TYPE "public"."enum_products_category" AS ENUM('interior', 'exterior', 'sliding', 'french', 'barn', 'custom');
  CREATE TYPE "public"."enum_products_status" AS ENUM('available', 'out-of-stock', 'custom-order');
  ...
  CREATE TABLE "products" (
  	...
  	"specifications_material" varchar NOT NULL,
  	"specifications_dimensions" varchar NOT NULL,
  	"specifications_style" varchar NOT NULL,
  	"specifications_finish" varchar NOT NULL,
  	"status" "enum_products_status" DEFAULT 'available',
  	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL
  );
